...
async function loadBootstrap(): Promise<BootstrapManifest> {
  try {
    const response = await fetch('/assets/bootstrap.json', { headers: { Accept: 'application/json' } });
    if (!response.ok) return {};
    return await parseJson<BootstrapManifest>(response);
  } catch {
    return {};
  }
}
...
export async function login(phone: string, password: string): Promise<{ ok: boolean; error?: string }> {
  ...
  const tenantId = (localStorage.getItem(SESSION_STORAGE.tenant) || boot.tenant_id || '').trim();
  const subId = (localStorage.getItem(SESSION_STORAGE.subtenant) || boot.subtenant_id || 'main').trim() || 'main';
  const headers: Record<string, string> = { ...authHeaders(false) as Record<string, string> };
  if (tenantId) {
    headers['X-Tenant-ID'] = tenantId;
    headers['X-Subtenant-ID'] = subId;
  }
