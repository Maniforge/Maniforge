# Maniforge Platform Core — документация

Документация для **развёртывания на инфраструктуре заказчика** (on-premise Production Box). Стартовые точки для покупателя и DevOps:

| Документ | Содержание |
|----------|------------|
| [PRODUCTION_BOX.md](PRODUCTION_BOX.md) | Production Box — установка, verify, backup, upgrade |
| [PRODUCTION_PLAN.md](PRODUCTION_PLAN.md) | Фазы до production (A–D) |
| [superpowers/plans/2026-09-17-api-holes-and-gateway.md](superpowers/plans/2026-09-17-api-holes-and-gateway.md) | Supply chain на шлюзе (8098–8101) в репо; Tasks 2–6 в коде; DoD — публичный gateway health и `go test` с Postgres |
| [MANIFORGE_PLATFORM_OVERVIEW.md](MANIFORGE_PLATFORM_OVERVIEW.md) | Обзор платформы |
| [MANIFORGE_ARCHITECTURE.md](MANIFORGE_ARCHITECTURE.md) | Архитектура и границы сервисов |
| [MANIFORGE_NEW_USER_WORKFLOW.md](MANIFORGE_NEW_USER_WORKFLOW.md) | Onboarding администратора |
| [MANIFORGE_ENTERPRISE_HARDENING.md](MANIFORGE_ENTERPRISE_HARDENING.md) | Production profile |
| [152FZ_COMPLIANCE.md](152FZ_COMPLIANCE.md) | ПДн (reference) |
| [legal/](legal/) | Шаблоны compliance |
| [openapi/](openapi/) | OpenAPI RBAC и Tenant Licensing |
| [../deploy/README.md](../deploy/README.md) | Операционные команды |

Сервисы platform core:

- [MANIFORGE_MANIFEST_ENGINE.md](MANIFORGE_MANIFEST_ENGINE.md)
- [MANIFORGE_TENANT_LICENSING_SERVICE.md](MANIFORGE_TENANT_LICENSING_SERVICE.md)
- [MANIFORGE_REALTIME.md](MANIFORGE_REALTIME.md)

Архитектурные решения: [adr/](adr/).
