...
| `cmd/warehouses/main.go` | HTTP Warehouses (`:8098`, prefix `/warehouses`) |
| `cmd/products/main.go` | HTTP Products (`:8099`, prefix `/products`) |
| `cmd/inventory/main.go` | HTTP Inventory posting/undo (`:8100`, prefix `/inventory`) |
| `cmd/wms/main.go` | HTTP WMS packs/КИЗ/scan (`:8101`, prefix `/wms`) |
...
| `internal/rbac/handler/pd_admin.go` | HTTP: PD admin (operator-profile, purposes, subject-requests, dpa-acknowledge) |
| `internal/rbac/handler/pd_self.go` | HTTP: `/me/personal-data*` (export, consents, subject-requests) |
...
| `internal/warehouses/` | Fiber: stocks CRUD, tree, types, audit, soft-archive |
| `internal/products/` | Fiber: SKU CRUD, EAN-13, archive |
| `internal/inventory/` | Fiber: balances, movements post/reverse/draft, reserves, lots, orders |
| `internal/wms/` | Fiber: packs, markings, scan, scan-movement |
| `internal/supplychain/` | Session guard, scope SQL, qty helpers |
| `migrations/pg/025_wms.sql` | WMS tables + barcode_ean13 + wms.* permissions |
...
| `internal/rbac/service/pd_self.go` | Self-service ПДн: export, consents, subject-requests |
...
| `internal/tenantlicensing/repository/extras.go` | Plans write, licenses list/patch/revoke, grants, quota, ops, audit |
| `internal/tenantlicensing/repository/repository_test.go` | Unit-тесты логики лицензии |
| `migrations/pg/026_pd_self_permissions.sql` | me.personal_data.* / me.consent.* для субъекта ПДн |
...
## Deploy (systemd, production server)

| Файл | Бинарник |
|------|----------|
| `deploy/systemd/maniforge-warehouses.service` | `bin/maniforge-warehouses` (`:8098`) |
| `deploy/systemd/maniforge-products.service` | `bin/maniforge-products` (`:8099`) |
