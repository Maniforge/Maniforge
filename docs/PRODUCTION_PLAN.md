...
| 2026-09-18 | Закрыты дыры тестов: `go test -p 1`, CSRF logout 403, negatives TL/Manifest/Versioning, `already_reversed`, concurrent issue + `SELECT FOR UPDATE`, парсинг Caddyfile 8098–8101. Versioning 403 больше не затирается 200 (`httpx.Fail` возвращал nil). | Живой `make platform-up` / Caddy на хосте — Skip, если :8080/:18090 нет |
