...
func NewApp(cfg config.Config, sqlDB *sql.DB) *fiber.App {
	...
	if sqlDB != nil {
		...
		register = func(router fiber.Router) {
			...
			api.Get("/tenants/:tenantCode/subtenants", h.ListSubtenants)
			api.Post("/tenants/:tenantCode/subtenants", h.CreateSubtenant)
			api.Patch("/tenants/:tenantCode/subtenants/:subtenantCode", h.UpdateSubtenant)
			api.Get("/tenants/:tenantCode/managed-tenants", h.ListManagedTenants)
			api.Post("/tenants/:tenantCode/managed-tenants", h.CreateManagedTenant)
			api.Delete("/tenants/:tenantCode/managed-tenants/:managedCode", h.RevokeManagedTenant)
			api.Post("/tenants/:tenantCode/managed-tenants/:managedCode/revoke", h.RevokeManagedTenant)
			api.Get("/plans", h.Plans)
			api.Post("/plans", h.UpsertPlan)
			api.Patch("/plans/:code", h.UpsertPlan)
			api.Get("/licenses", h.ListLicenses)
			api.Post("/licenses/assign", h.AssignLicense)
			api.Patch("/licenses/:id", h.UpdateLicense)
			api.Post("/licenses/revoke", h.RevokeLicense)
			api.Get("/tenants/:tenantCode/entitlements", h.Entitlements)
			api.Get("/tenants/:tenantCode/quota", h.Quota)
			api.Get("/ops/summary", h.OpsSummary)
