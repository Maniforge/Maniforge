...
func NewApp(cfg config.Config, sqlDB *sql.DB) *fiber.App {
	...
	pdSelfHandler := handler.NewPDSelfHandler(cfg, sqlDB)
	...
	register := func(router fiber.Router) {
		...
		protected.Post("/auth/switch-project", authHandler.SwitchProject)
		protected.Post("/auth/accept-invite", authHandler.AcceptInvite)
		...
		protected.Post("/me/security/password", meHandler.ChangePassword)
		protected.Get("/me/personal-data", pdSelfHandler.ExportMe)
		protected.Get("/me/personal-data/consents", pdSelfHandler.ListConsents)
		protected.Post("/me/personal-data/consents", pdSelfHandler.GrantConsent)
		protected.Post("/me/personal-data/consents/revoke", pdSelfHandler.RevokeConsent)
		protected.Get("/me/personal-data/subject-requests", pdSelfHandler.ListSubjectRequests)
		protected.Post("/me/personal-data/subject-requests", pdSelfHandler.CreateSubjectRequest)
		...
		protected.Post("/projects/memberships", projectsHandler.AssignMembership)
		protected.Get("/projects/:code", projectsHandler.Get)
		protected.Patch("/projects/:code", projectsHandler.Patch)
		protected.Get("/global-variables", projectsHandler.ListGlobalVariables)
		...
		protected.Patch("/admin/users", adminHandler.UpdateUser)
		protected.Delete("/admin/users", adminHandler.DeleteUser)
		...
		protected.Post("/admin/user-roles/revoke", adminHandler.RevokeUserRole)
		protected.Post("/admin/user-roles/batch", adminHandler.BatchUserRoles)
		...
		protected.Post("/admin/personal-data/dpa-acknowledge", pdAdminHandler.AcknowledgeDPA)
		protected.Post("/admin/registration-invites", adminHandler.CreateRegistrationInvite)
		protected.Post("/admin/organization-members", adminHandler.AttachOrganizationMember)
