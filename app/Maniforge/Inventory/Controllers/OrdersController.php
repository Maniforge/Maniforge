...
final class OrdersController
{
    ...
    public function list(RequestContext $ctx): void
    {
        ...
        $result = $this->orders->list($session, $this->queryParams($ctx));
    ...
    public function create(RequestContext $ctx): void
    {
        ...
        $result = $this->orders->create($session, $ctx->input);
        JsonResponse::send($result, (int) $result['status']);
    }
    ...
    public function get(RequestContext $ctx, int $id): void
    {
        ...
        $result = $this->orders->get($session, $id);
        JsonResponse::send($result, (int) $result['status']);
    }

    public function confirm(RequestContext $ctx, int $id): void
    {
        $session = $this->access->guardWrite($ctx);
        if ($session === null) {
            return;
        }
        $result = $this->orders->confirm($session, $id);
        JsonResponse::send($result, (int) $result['status']);
    }

    public function fulfill(RequestContext $ctx, int $id): void
    {
        $session = $this->access->guardWrite($ctx);
        if ($session === null) {
            return;
        }
        $result = $this->orders->fulfill($session, $id);
        JsonResponse::send($result, (int) $result['status']);
    }

    public function cancel(RequestContext $ctx, int $id): void
    {
        $session = $this->access->guardWrite($ctx);
        if ($session === null) {
            return;
        }
        $result = $this->orders->cancel($session, $id);
        JsonResponse::send($result, (int) $result['status']);
    }
